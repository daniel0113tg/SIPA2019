...
require_once'../envio-phpmailer/mail.php';
...
function envia_mail_credenciales_p($correo,$usuario,$contrasenia,$nombre,$apellido){
enviarCorreo($correo,$usuario,$contrasenia,$nombre,$apellido);
}
...
function insertar_profesor($ci, $nombres, $apellidos, $id_departamento, $id_facultad, $mail, $id_usuario, $ruta_imagen) {
    $conexion = new Conexion();
    $statement = 'INSERT INTO profesor (ci,nombres,apellidos,id_departamento, id_facultad, mail, id_usuario, ruta_imagen) VALUES (?,?,?,?,?,?,?,?)';
    $consulta = $conexion->prepare($statement);
    if ($consulta->execute(array($ci, $nombres, $apellidos, $id_departamento, $id_facultad, $mail, $id_usuario, $ruta_imagen))) {
        return true;
    } else {
        return false;
    }
...
function obtener_imagen_pro($usuario) {
    $conexion = new Conexion();
    $statement = 'select pro.ruta_imagen from usuario as us, profesor as pro where us.idUsuario=pro.id_usuario and us.usuario=?';
    $consulta = $conexion->prepare($statement);
    $consulta->setFetchMode(PDO::FETCH_ASSOC);
    $consulta->execute([$usuario]);
    if ($consulta->rowCount() != 0) {
